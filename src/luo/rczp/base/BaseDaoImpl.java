...
public abstract class BaseDaoImpl<T,PK extends Serializable> implements BaseDao<T,PK> {
	...
	@SuppressWarnings("unchecked")
	protected Pagination findGroup(Finder finder, int pageNo, int pageSize) {
		//GROUP查询语句的总记录数不能按此方法得到,只能查询所有记录获取行数
		//int totalCount = countQueryResult(finder);
		Query query = getSession().createQuery(finder.getOrigHql());
		finder.setParamsToQuery(query);
	...
	public Object updateByUpdater(Updater updater) {
		...
		if (cm == null) {
			superClass = true;
			cm = getCmd(updater.getBean().getClass().getSuperclass());
		}
		if (cm == null) {
			throw new RuntimeException("所更新的对象没有映射或不是实体对象");
		}
		...
		Object po = null;
		if(superClass){
			po = getSession().load(bean.getClass().getSuperclass(),
					cm.getIdentifier(bean, POJO));
		}else{
			po = getSession().load(bean.getClass(),
					cm.getIdentifier(bean, POJO));
		}
		updaterCopyToPersistentObject(updater, po);
		return po;
